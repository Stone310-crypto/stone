...
pub async fn handle_upload_document(
    ...
    // ── Shard-Verteilung VOR dem Block-Commit ────────────────────────────────
    // Shards werden synchron an Peers gesendet, damit die aktualisierten
    // Holder-Infos bereits im Block stehen.
    let chunks = if let Some(ref network) = state.network {
        let has_shards = chunks.iter().any(|c| !c.shards.is_empty());
        if has_shards {
            match stone::shard::ShardStore::new() {
                Ok(shard_store) => {
                    // distribute_shards gibt ChunkRefs mit aktualisierten Holdern zurück
                    ...
                    // Shard-Holder-Registry aktualisieren
                    let local_peer_id = network.local_peer_id.to_string();
                    for cr in &updated {
                        if cr.ec_k > 0 {
                            // Lokale Shards registrieren
                            state.node.shard_registry.register_local_shards(
                                &cr.hash,
                                cr.ec_k + cr.ec_m,
                                &local_peer_id,
                            );
                            // Remote-Holder registrieren (aus den aktualisierten ShardRefs)
                            for sr in &cr.shards {
                                for holder in sr.holder.split(',').filter(|h| !h.is_empty()) {
                                    state.node.shard_registry.add_holder(&cr.hash, sr.shard_index, holder);
                                }
                            }
                        }
                    }
                    state.node.shard_registry.persist();
                    println!("[sharding] ✅ {} Chunks verteilt + Registry aktualisiert", updated.len());
                    updated
                }
                Err(e) => {
                    eprintln!("[sharding] ShardStore öffnen fehlgeschlagen: {e}");
                    chunks  // Fallback: Original-Chunks ohne Verteilung
                }
            }
        } else {
            chunks
