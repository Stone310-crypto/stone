...
pub async fn handle_qr_status(State(state): State<AppState>, Path(login_token): Path<String>) -> impl IntoResponse {
    ...
    if let Some(approved) = poll_peers_for_qr_session(&state, &login_token).await {
        let user = stone::auth::User { id: approved.user_id.clone(), name: approved.user_name.clone(), bio: String::new(), api_key: String::new(), phrase_hash: String::new(), quota_bytes: stone::auth::default_quota_bytes(), wallet_address: approved.wallet_address.clone(), account_type: approved.account_type.clone(), org_id: String::new(), org_role: String::new(), discord_id: approved.discord_id.clone(), discord_username: approved.discord_username.clone(), vpn_id: None, updated_at: chrono::Utc::now().timestamp() };
