...
pub async fn handle_mining_status(
    ...
    // Staking-Infos
    let (total_staked, staker_count) = {
        let pool = state.node.staking_pool.read().unwrap();
        (pool.total_staked.to_string(), pool.stakers.len())
    };
    ...
    let (total_supply, circulating, validator_balance, pool_balance, validator_wallet) = {
        ...
        (
            ledger.total_supply().to_string(),
            ledger.total_supply().to_string(),
            ledger.balance(&vw).to_string(),
            ledger.balance("pool:storage_rewards").to_string(),
            vw,
        )
    };
...
pub async fn handle_mining_stake(
    ...
    // Balance der User-Wallet prüfen
    let (balance, nonce) = {
        let ledger = state.node.token_ledger.read().unwrap();
        (ledger.balance(&user_wallet), ledger.nonce(&user_wallet))
    };
    ...
    if balance < amount {
        return (
            ...
            axum::Json(json!({
                "ok": false,
                "error": format!("Nicht genug Balance: {} verfügbar, {} angefordert", balance, amount),
            })),
        ).into_response();
    }

    // Validator-Key zum Signieren (Signatur-Check für Stake-TXs übersprungen)
    let signing_key = load_or_create_validator_key();
...
pub async fn handle_mining_unstake(
    ...
    let nonce = {
        let ledger = state.node.token_ledger.read().unwrap();
        ledger.nonce(&user_wallet)
    };

    // Validator-Key zum Signieren (Signatur-Check für Unstake-TXs übersprungen)
    let signing_key = load_or_create_validator_key();
