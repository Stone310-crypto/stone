...
/** Lädt und rendert die Theme-Editor-Extension-UI in einem iframe. */
export default function ThemeEditorView() {
  const [ui, setUI] = useState<string | null>(null);

  useEffect(() => {
    invoke<string | null>("get_extension_ui", { id: "theme-editor" })
      .then(setUI)
      .catch(() => setUI(null));
  }, []);
  ...
  if (!ui) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--text-muted)", flexDirection: "column", gap: 8 }}>
        ...
        <span>Theme Editor ist nicht installiert.</span>
        <span style={{ fontSize: 12 }}>
          Installiere ihn im 🧩 Erweiterungen-Tab.
        </span>
      </div>
    );
  }
