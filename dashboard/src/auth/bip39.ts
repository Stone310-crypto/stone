...
const WORDLIST = [
  ...
  "turtle", "twelve", "twenty", "twice", "twin", "twist", "two", "ugly",
...
WORDLIST.forEach((w, i) => WORD_MAP.set(w, i));

export function mnemonicToEntropy(phrase: string): Uint8Array {
  const words = phrase.trim().toLowerCase().split(/\s+/);
  if (words.length !== 12 && words.length !== 24) {
    throw new Error(`Invalid word count: ${words.length} (expected 12 or 24)`);
  }
  const checksumBits = words.length / 3;
  const entropyBytes = (words.length * 11 - checksumBits) / 8;
  let bits = 0n;
  for (const word of words) {
    const index = WORD_MAP.get(word);
    if (index === undefined) throw new Error(`Invalid BIP39 word: "${word}"`);
    bits = (bits << 11n) | BigInt(index);
  }
  bits >>= BigInt(checksumBits);
  const entropy = new Uint8Array(entropyBytes);
  let remaining = bits;
  for (let i = entropyBytes - 1; i >= 0; i--) {
    entropy[i] = Number(remaining & 0xffn);
    remaining >>= 8n;
  }
  return entropy;
}

export async function deriveEd25519Seed(phrase: string): Promise<Uint8Array> {
...
export async function signChallenge(challenge: string, phrase: string): Promise<string> {
  ...
  const privKey = await crypto.subtle.importKey("pkcs8", pkcs8.buffer as ArrayBuffer, { name: "Ed25519" }, false, ["sign"]);
  const chalBytes = new TextEncoder().encode(challenge);
  const sig = await crypto.subtle.sign("Ed25519", privKey, chalBytes.buffer as ArrayBuffer);
